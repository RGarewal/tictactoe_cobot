...
class Object_detect():
    ...
    def __init__(self, camera_x=160, camera_y=15):
        ...
        self.HSV = {
            ...
            "red": [np.array([35, 43, 35]), np.array([90, 255, 255])],
...
def draw_board(frame):
    ...
    range_x = [cell_width * i for i in range(COLS)]
    range_y = [cell_height * i for i in range(ROWS)]

    for i in range(1, max(ROWS, COLS)):
        if i < ROWS:
            cv2.line(frame, (0, i * cell_height), (width, i * cell_height), (0, 0, 0), 2)
        if i < COLS:
            cv2.line(frame, (i * cell_width, 0), (i * cell_width, height), (0, 0, 0), 2)
...
def play_tic_tac_toe_with_camera(detect, cap):
    ...
    while True:
        ...
        message = "Your turn" if player_turn else "Computer's turn"
        cv2.putText(frame, message, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
        ...
        if key == ord(' ') and player_turn:  
            ...
            for center in blue_centers:
                ...
                if board[yVal][xVal] == ' ':
                    board[yVal][xVal] = current_player
